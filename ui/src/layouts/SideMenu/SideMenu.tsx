...
import { useMenuOpened, useMenuOpen, useMenuClose } from '@stores/navigationStore'
...
function SystemStats({ expanded }: { expanded: boolean }) {
  ...
  if (import.meta.env.DEV && data) {
    console.log('[SystemStats] snapshot shape:', JSON.stringify(Object.keys(data)))
    console.log('[SystemStats] gpus field:', data.gpus)
  }

  // Color based on utilization
  const color = (pct: number) =>
    pct >= 90 ? 'var(--mantine-color-red-5)'
    : pct >= 70 ? 'var(--mantine-color-yellow-5)'
      ...
      {gpu ? (
        <>
          <Flex justify="space-between">
            ...
            <Text size="xs" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', fontWeight: 600, color: color(gpu.utilization.gpuPercent) }}>
          ...
          <Flex justify="space-between">
            ...
            <Text size="xs" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', fontWeight: 600, color: color((gpu.vram.usedMiB / gpu.vram.totalMiB) * 100) }}>
          ...
          <Flex justify="space-between">
            ...
            <Text size="xs" style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', fontWeight: 600, color: gpu.temperatureCelsius >= 85 ? 'var(--mantine-color-red-5)' : undefined }}>
...
function RecursiveNavLink({
  ...
  const isActive = block.link ? pathname.startsWith(block.link) : false
  ...
  const navLink = (
    <NavLink
      ...
      onClick={() => onToggle(path, hasChildren)}
      styles={{
        root: {
          height: 40,
          borderRadius: 6,
        //   paddingLeft: depth === 0 ? (sidebarOpened ? 12 : 0) : undefined,
        //   justifyContent: depth === 0 && !sidebarOpened ? 'center' : undefined,
  ...
  // Wrap top-level items with tooltip when collapsed
  if (depth === 0 && !sidebarOpened) {
    return (
      <Tooltip label={block.disabled ? `${block.label} (coming soon)` : block.label} position="right" withArrow offset={8}>
        <div>{navLink}</div>
      </Tooltip>
    )
  }
...
export function SideMenu({ blocks }: SideMenuProps) {
  ...
  useEffect(() => {
    if (!opened) setExpandedItems(new Set())
  }, [opened])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Tooltip label={opened ? 'Collapse' : 'Expand'} position="right" withArrow disabled={opened}>
          <ActionIcon
            variant="subtle"
            color="gray"
            size="sm"
