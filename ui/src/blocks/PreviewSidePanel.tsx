...
import { ARCH_COLOR, ARCH_LABEL, type ModelArchitecture } from '@services/models'
 ...
 */
function parseActivationToken(prompt: string): { token: string; rest: string; sep: string } | null {
 ...
 */
function findPairPrompt(prompts: SamplePrompt[], current: SamplePrompt): SamplePrompt | null {
...
function PromptText({ prompt }: { prompt: string }) {
  ...
  if (!parsed) {
    return (
      <Text size="sm" style={{ fontFamily: 'var(--mantine-font-family-monospace)',
        fontSize: '0.78rem', lineHeight: 1.6, wordBreak: 'break-word' }}>
        {prompt}
      </Text>
    )
  }
  ...
  return (
    <Text size="sm" style={{ fontFamily: 'var(--mantine-font-family-monospace)',
      ...
      {/* Separator dimmed */}
      <Text component="span" c="dimmed">{parsed.sep} </Text>
...
function CompareModal({
  opened, onClose, urlWithToken, urlNoToken, tokenLabel, aspectRatio: ar,
}: CompareModalProps) {
  return (
    <Modal
      opened={opened}
      ...
      title={
        <Group gap="xs">
          ...
          <Text fw={600} size="sm">Token comparison</Text>
        </Group>
      }
      size="xl"
      centered
    ...
    >
      <Stack gap="sm">
        ...
        <Group justify="space-between" px={2}>
          <Badge variant="filled" color="orange" size="sm" style={{ letterSpacing: '0.02em' }}>
            ← {tokenLabel}
          </Badge>
        ...
        <Compare
          leftSection={
            <img src={urlWithToken} alt="with token"
              style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          }
          rightSection={
            <img src={urlNoToken} alt="without token"
              style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          }
          aspectRatio={ar}
          variant="drag"
...
export function PreviewSidePanel({
  image, meta, prompt, arch, dimensions, onClose,
  jobId, checkpointPreviews, prompts,
  ...
  const pairPreview = pairPrompt !== null
    ? checkpointPreviews.find(p => p.promptIndex === pairPrompt.index)
    : null
  const pairUrl     = pairPreview ? outputsApi.getPreviewUrl(jobId, pairPreview.filename) : null
        ...
        <Stack gap="md">
          ...
          <Group gap="xs" wrap="wrap">
            ...
            {meta.epoch !== undefined && (
              <Badge size="xs" variant="outline" color="gray">epoch {meta.epoch}</Badge>
            )}
            {meta.step !== undefined && (
            ...
            {dimensions && (
              <Badge size="xs" variant="outline" color="gray">
                {dimensions.w}×{dimensions.h} · {ratioStr(dimensions.w, dimensions.h)}
              </Badge>
            )}
          ...
          <SimpleGrid cols={3} spacing="xs">
            ...
            <StatCell
              label="Ratio"
              value={prompt.width && prompt.height ? ratioStr(prompt.width, prompt.height) : undefined}
            />
          </SimpleGrid>
...
export function makePreviewSidePanel({
  metas, prompts, arch, onClose, jobId, checkpointPreviews,
