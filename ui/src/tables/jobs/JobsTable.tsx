import { ActionIcon, Badge, Center, Group, Text, Tooltip } from "@mantine/core";
...
import type { JobSummaryResponseDto } from "@api/types.gen";
...
interface JobsTableProps {
  onView: (job: JobSummaryResponseDto) => void;
  onViewOutputs: (job: JobSummaryResponseDto) => void;
}
...
export default function JobsTable({ onView, onViewOutputs }: JobsTableProps) {
  ...
  const confirmKill = useCallback(
    (job: JobSummaryResponseDto) => {
      openConfirmModal({
        title: "Kill Job",
        centered: true,
        children: (
          <Text size="sm">
            Stop training job <strong>{job.name}</strong>? The process will
            receive SIGTERM.
          </Text>
        ),
        labels: { confirm: "Kill", cancel: "Cancel" },
        confirmProps: { color: "red" },
        onConfirm: () => killJob({ id: job.id }),
      });
    },
    [killJob],
  );
  ...
  const columns: DataTableColumn<JobSummaryResponseDto>[] = useMemo(
    () => [
      {
        accessor: "status",
        title: "Status",
      ...
      },
      {
        ...
        render: (job) => (
          <Group gap={4} justify="right" wrap="nowrap">
            ...
            <Tooltip
              label={isActive(job.status) ? "Kill job" : "Already finished"}
              withArrow
            >
              <ActionIcon
                size="sm"
    ...
    [onView, confirmKill, onViewOutputs],
  );
  ...
  return (
    <DataTable<JobSummaryResponseDto>
      idAccessor="id"
