...
export class DatasetsController {
    ...
    schema: {
      ...
      properties: {
        ...
        file: {
          type: "string",
          format: "binary",
          description: "Zip archive (max 500 MB)",
        },
      },
    },
  })
  @ApiQuery({
    ...
    required: false,
    type: Boolean,
    description: "Merge into existing dataset (default: true)",
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 400,
    description: "Invalid dataset name or missing fields",
  })
  @ApiResponse({
    status: 409,
    description: "Dataset already exists and overwrite=false",
  })
  @UseInterceptors(
  ...
  async uploadZip(
    ...
    return this.datasetsService.uploadZip(
      name,
      file.buffer,
      overwrite !== "false",
    );
    ...
    schema: {
      ...
      properties: {
        file: {
          type: "string",
          format: "binary",
          description: "Image or caption file (max 50 MB)",
        },
      },
    },
  })
  ...
  @ApiResponse({
    status: 400,
    description: "Missing query param or unsupported file type",
  })
  @UseInterceptors(
    FileInterceptor("file", {
      storage: memoryStorage(),
  ...
  @ApiResponse({
    ...
    type: InitChunkedUploadResponseDto,
  })
  ...
  @ApiOperation({
    summary: "Upload a single chunk to an active upload session",
  })
  @ApiConsumes("multipart/form-data")
  @ApiBody({
    schema: {
      ...
      properties: {
        file: {
          type: "string",
          format: "binary",
          description: "Chunk binary data (max 10 MB)",
        },
  ...
  @ApiResponse({
    ...
    type: SaveChunkResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: "Unknown session or invalid chunkIndex",
  })
  @UseInterceptors(
    FileInterceptor("file", {
      storage: memoryStorage(),
  ...
  async uploadChunk(
    @UploadedFile(
      new ParseFilePipe({
        validators: [
          new MaxFileSizeValidator({ maxSize: CHUNK_MAX_SIZE_BYTES }),
        ],
      }),
    )
    file: Express.Multer.File,
    ...
    if (!uploadId)
      throw new BadRequestException('Body field "uploadId" is required');
    if (!chunkIndexStr)
      throw new BadRequestException('Body field "chunkIndex" is required');
  ...
  @ApiResponse({
    status: 400,
    description: "Session not found, missing chunk, or invalid body",
  })
  @ApiResponse({
    status: 409,
    description: "Dataset already exists and overwrite=false",
  ...
  async completeChunkedUpload(
    ...
    if (!totalChunks || typeof totalChunks !== "number" || totalChunks < 1) {
      throw new BadRequestException(
        'Body field "totalChunks" must be a positive integer',
      );
    }

    return this.datasetsService.completeChunkedUpload(
      uploadId,
      name,
      totalChunks,
      overwrite !== "false",
  ...
  @ApiResponse({
    status: 200,
    description: "Array of dataset summaries",
  ...
  @ApiOperation({
    summary: "Serve an image file from a dataset (no auth required)",
  })
  @ApiParam({ name: "name", description: "Dataset directory name" })
  @ApiParam({
    name: "filename",
    description: "Image filename, e.g. my_char_001.jpg",
  })
  @ApiResponse({
    status: 200,
    description: "Raw image bytes with correct Content-Type",
  })
  ...
  async serveImage(
    ...
    const imagePath = await this.datasetsService.resolveImagePath(
      name,
      safeFilename,
    );
    try {
  ...
  @ApiParam({
    name: "filename",
    description: "Filename to replace, e.g. my_char_001.jpg",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiResponse({
    ...
    type: ReplaceImageResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: "Extension mismatch or unsupported filename",
  })
  @ApiResponse({ status: 404, description: "Dataset not found" })
  @UseInterceptors(
    FileInterceptor("file", {
  ...
  @ApiResponse({
    status: 200,
    description: "Prepend result",
    type: PrependTokenResultDto,
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Caption text and length stats",
    type: GetCaptionResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: "Filename is not a supported image format",
  })
  @ApiResponse({ status: 404, description: "Dataset not found" })
  async getCaption(
    @Param("name") name: string,
  ...
  @ApiResponse({
    status: 200,
    description: "Caption written, stats returned",
    type: UpsertCaptionResponseDto,
  })
  @ApiResponse({
    status: 400,
    description: "Missing caption text or unsupported image format",
  })
  ...
  async upsertCaption(
    ...
    return this.datasetsService.upsertCaption(
      name,
      path.basename(image),
      caption,
    );
  ...
  @ApiResponse({
    status: 200,
    description: "Metadata record or null",
    type: DatasetMetaDto,
  })
  @ApiResponse({ status: 404, description: "Dataset not found" })
  ...
  @ApiResponse({
    status: 200,
    description: "Updated metadata record",
    type: DatasetMetaDto,
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Detection result with updated metadata",
    type: DetectCaptionTypeResultDto,
  })
  @ApiResponse({ status: 404, description: "Dataset not found" })
  async detectCaptionType(
    @Param("name") name: string,
  ): Promise<DetectCaptionTypeResultDto> {
