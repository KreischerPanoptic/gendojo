...
/** Parse a numeric string; returns null (not NaN) on failure or [N/A] input */
...
export class SystemService implements OnModuleInit, OnModuleDestroy {
  ...
  /** Most recently collected snapshot — returned by GET /system without waiting */
  private snapshot: SystemSnapshot | null = null;

  /** Previous /proc/stat values for delta-based CPU utilisation calculation */
  private prevCpuTimes: [number, number] | null = null; // [total, idle]

  /**
   * Guard against concurrent polls.
   * nvidia-smi can occasionally block for 10+ seconds; without this guard
   * multiple poll() calls stack up and hammer the system simultaneously.
   */
  ...
  private async collectGpus(): Promise<GpuInfo[]> {
    ...
    for (const line of lines) {
      ...
      const [
        indexStr, name, uuid, driverVersion,
  ...
  private async collectDisks(): Promise<DiskInfo[]> {
    ...
    for (const line of stdout.trim().split('\n').slice(1)) {
      ...
      let sizeRaw: string, usedRaw: string, availRaw: string, pcentRaw: string;

      if (isGnuOutput) {
        [source, target, sizeRaw, usedRaw, availRaw, pcentRaw] = parts as [string, string, string, string, string, string];
      } else {
        // POSIX: Filesystem 1024-blocks Used Available Use% Mounted
        [source, sizeRaw, usedRaw, availRaw, pcentRaw, target] = parts as [string, string, string, string, string, string];
      }
